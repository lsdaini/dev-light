...
<article class="page-container">
	<form method="post" class="form form-horizontal" id="form-role-update">
		<div class="row cl">
			...
			<div id="menus" class="formControls col-xs-8 col-sm-9">
				<c:forEach var="menu" items="${parentList }">
					<dl class="permission-list">
						<dt>
							<label>
								<input type="checkbox" value="${menu.id }" <c:if test="${fn:contains(menus,menu.id)}">checked</c:if>>
								${menu.name }
							</label>
						</dt>
							...
							<dd>
								<c:forEach var="child" items="${menu.childs }">
									<dl class="cl permission-list2">
										<dt>
											<label class="">
												<input type="checkbox" value="${child.id }" <c:if test="${fn:contains(menus,child.id)}">checked</c:if>>
												${child.name }
												</label>
										</dt>
									</dl>
...
$(function(){
	...
	$("#form-role-update").validate({
		...
		submitHandler:function(form){
			...
			$.post("roleUpdate", param, function(data){
				if(data.success){
